...
Function ParseFormula(formula As String, cell As range, sheet As Worksheet, ByRef keys)
    ...
    For i = 1 To VBA.Len(formula)
        ...
        ElseIf quotes Then
            If VBA.Len(fn) > 0 Then
                fn = fn & char
            End If
        ElseIf parens = 0 And inFNBX = 0 And (char = "F" Or char = "f") Then
            fn = fn & char
            inFNBX = 1
        ElseIf parens = 0 And inFNBX = 1 And (char = "N" Or char = "n") Then
            fn = fn & char
            inFNBX = 2
        ElseIf parens = 0 And inFNBX = 2 And (char = "B" Or char = "b") Then
            fn = fn & char
            inFNBX = 3
        ElseIf parens = 0 And inFNBX = 3 And (char = "X" Or char = "x") Then
            fn = fn & char
            inFNBX = 4
    ...
    If hasFNBX And resolvable Then
...
Function ParseKeys(formula As String, cell As range, sheet As Worksheet, ByRef keys) As String
    ...
    If name = "FNBX" Or name = "=FNBX" Or name = "=-FNBX" Then
        ...
        nested = False
        If argsCount > 0 Then
            If VBA.InStr(VBA.UCase(args(0)), "FNBX(") > 0 Then
        ...
        If argsCount > 1 Then
            If VBA.InStr(VBA.UCase(args(1)), "FNBX(") > 0 Then
        ...
        If argsCount > 2 Then
            If VBA.InStr(VBA.UCase(args(2)), "FNBX(") > 0 Then
        ...
        If argsCount > 3 Then
            If VBA.InStr(VBA.UCase(args(3)), "FNBX(") > 0 Then
        ...
        If IsCached(key) And Not IsCachedError(key) Then
            ParseKeys = CachedToFNBX(key, index)
        End If
